...
gapminder <- 
  ...
  mutate(pop_rank = rank(population)) |>  # rank order populations
  ungroup()
...
```{r}
life_exp_se_est <- sd(life_exp_2000s) / length(life_exp_2000s)
life_exp_se_est
```
...
Who knows. Maybe the sample mean we're collecting is actually overestimating the true mean. We cannot know for sure. But, we can create an interval which we expect to contain the true mean with some defined level of confidence ...
