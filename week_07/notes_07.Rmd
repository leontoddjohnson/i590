...
**A null hypothesis is a *neutral* and *quantitative* statement about a population parameter that *may* be disproved.**
...
+--------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------+
| *"The BMI for active athletes is better than the BMI for non-athletes."*       | *"The mean difference in BMI between athletes and no-athletes is 0."*                                                  |
+--------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------+
| *"House prices increase each year"*                                            | *"The mean house price for year* $t$ *is equal to the mean house price for year* $t-1$*."*                             |
+--------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------+
...
2.  Devise a null hypotheses based on the documentation which incorporates two columns of data.
...
**Effect size quantifies the difference between the null hypothesis and the alternative hypothesis.** You can think of it as a sort of "signal-to-noise" measure for the difference between two situations.
...
Roughly, this number can range from a "negligible" effect (i.e., $|d| < 0.2$) to a "large" effect (i.e., $|d| \geq 0.8$). For larger values, hypothesis testing might not be needed at all given the vast difference, whereas small values typically support the effort of hypothesis testing.

> *Note: Cohen's D (along with many other statistics) do not hold up well for small samples of data less than 100.*
...
7.  Run the hypothesis test, and draw conclusions.
